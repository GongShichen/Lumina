...
public final class CoreMLJSONPlannerModel: LocalStructuredInferenceModel, @unchecked Sendable {
    ...
    public func generateJSON(prompt: String) async throws -> String {
        ...
        guard let value = output.featureValue(for: configuration.jsonOutputName)?.stringValue else {
            throw CoreMLPlannerError.missingStringOutput(configuration.jsonOutputName)
        }
        return value
    }
}

public typealias CoreMLTextToJSONModel = CoreMLJSONPlannerModel

public enum CoreMLPlannerError: LocalizedError {
