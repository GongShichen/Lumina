...
enum LuminaAgenticRLSuite {
    ...
    private static let templates: [Template] = [
        ...
        Template(instruction: "帮我规划明天下午的 LuminaTest 深度工作：先查日历冲突，再把可行时间段保存成一篇 Markdown 笔记，最后把摘要写入本地记忆。", category: "planning_memory", difficulty: "hard", tools: ["device.current_time", "calendar.availability", "file.save_note", "memory.ingest_text"]),
        ...
        Template(instruction: "读取剪贴板中的文本，提取其中可能的待办事项，把结果整理成 Markdown 笔记 LuminaTest-clipboard-todos.md，并存入本地记忆。", category: "content_memory", difficulty: "medium", tools: ["clipboard.read", "text.transform", "file.save_note", "memory.ingest_text"]),
        ...
        Template(instruction: "订阅 https://example.com/feed.xml，标题使用 LuminaTest Example Feed；订阅后刷新一次，并把刷新结果摘要写入记忆。", category: "subscription", difficulty: "hard", tools: ["subscription.add", "subscription.refresh", "memory.ingest_text"]),
        ...
        Template(instruction: "把 https://example.com 的正文抓取下来，提炼 3 个要点，保存为 LuminaTest-example-summary.md，并把来源 URL 一起写入本地记忆。", category: "web_memory", difficulty: "hard", tools: ["webpage.fetch_text", "text.transform", "file.save_note", "memory.ingest_text"]),
        ...
        Template(instruction: "搜索本地记忆中关于 LuminaTest benchmark 的内容，找出最近 3 条，并生成一个简短回顾，不要新增记忆。", category: "memory_retrieval", difficulty: "medium", tools: ["local.search", "memory.recent"]),
        Template(instruction: "把一段 LuminaTest 训练记录写入本地记忆，然后立刻搜索它，确认它能被检索到；最终说明检索是否命中。", category: "memory_write_read", difficulty: "hard", tools: ["memory.ingest_text", "local.search"]),
        ...
        Template(instruction: "计算 42*7+18，然后把结果复制到剪贴板，并保存一条 LuminaTest 计算记录到本地记忆。", category: "calculator_memory", difficulty: "medium", tools: ["calculator.evaluate", "clipboard.write", "memory.ingest_text"]),
        ...
        Template(instruction: "查看当前位置和存储状态，生成一份 LuminaTest 设备上下文摘要并保存到本地记忆。", category: "device_memory", difficulty: "medium", tools: ["location.current", "storage.status", "memory.ingest_text"]),
        ...
        Template(instruction: "刷新订阅源，搜索新写入的本地记忆摘要，最后输出来源引用；如果刷新失败，说明原因并不要伪造结果。", category: "subscription_memory", difficulty: "hard", tools: ["subscription.refresh", "local.search"]),
        Template(instruction: "删除标题包含 LuminaTest 临时 的本地记忆，并输出删除数量；如果没有找到，明确说没有删除任何东西。", category: "memory_delete", difficulty: "medium", tools: ["local.search", "memory.delete"]),
        ...
        Template(instruction: "查询最近账目和最近本地记忆，判断我是否已经记录过 LuminaTest 咖啡；如果没有，就新增一条 22 元记录。", category: "cross_store_reasoning", difficulty: "hard", tools: ["ledger.search", "local.search", "ledger.record"]),
