...
enum LuminaAgenticRLSuite {
    ...
    private static let templates: [Template] = [
        ...
        Template(instruction: "把一段 LuminaTest 训练记录写入本地记忆，然后把同一段内容保存成 Markdown 文件；最终只说明写入和保存状态，不读取既有记忆。", category: "memory_write_file", difficulty: "hard", tools: ["memory.ingest_text", "file.save_note"]),
        Template(instruction: "基于当前时间、电量和网络状态生成一段 LuminaTest 运行记录，保存为 Markdown 文件，并把摘要写入本地记忆。", category: "memory_write_context", difficulty: "hard", tools: ["device.current_time", "device.power_status", "network.status", "file.save_note", "memory.ingest_text"]),
        ...
        Template(instruction: "刷新订阅源，并把刷新结果保存成 LuminaTest subscription refresh Markdown 文件；如果刷新失败，说明原因并不要伪造结果。", category: "subscription_file", difficulty: "hard", tools: ["subscription.refresh", "file.save_note"]),
