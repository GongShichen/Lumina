...
enum LuminaBenchmarkSuite {
    ...
    private static let sideEffectTools: Set<String> = [
        ...
        "share.prepare", "app.open_settings", "memory.ingest_text", "memory.delete",
    ...
    private static let templates: [Template] = [
        ...
        Template(text: "看一下最近的本地记忆", tools: ["memory.recent"], category: "memory"),
        Template(text: "本地记忆库现在有多少片段", tools: ["memory.stats"], category: "memory"),
        Template(text: "记住这段：LuminaTest benchmark 需要覆盖真实工具", tools: ["memory.ingest_text"], category: "memory"),
        Template(text: "删除刚才那条 LuminaTest benchmark 记忆", tools: ["memory.delete"], category: "memory"),
        Template(text: "搜索本地记忆里关于 LuminaTest benchmark 的内容", tools: ["local.search"], category: "memory"),
