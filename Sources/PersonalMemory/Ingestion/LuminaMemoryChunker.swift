...
public struct LuminaMemoryChunker: Sendable {
    ...
    public func chunks(for document: LuminaMemoryDocument) -> [LuminaMemoryChunk] {
        let text = document.body.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else { return [] }

        var results: [LuminaMemoryChunk] = []
        ...
        while start < text.endIndex {
            ...
            if !chunkText.isEmpty {
                results.append(LuminaMemoryChunk(
