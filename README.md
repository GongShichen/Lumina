...
工程目标不是做一个只能演示规划的 toy app，而是把“用户输入 -> agent 自主规划 -> 工具执行 -> 权限确认 -> 可读 Markdown 回复 -> 审计记录”做成完整闭环。敏感数据默认留在本地；需要副作用的动作会先经过 Lumina 自定义确认，再触发系统权限或系统界面。

## 架构

### AgentRuntime

`AgentRuntime` 是纯 runtime framework。它不包含 App 业务逻辑、不内置 memory 策略、不写 system prompt，也不绑定具体模型实现。App 或其它宿主通过 request、context provider、planner、tool registry、permission gate、confirmation coordinator、audit logger 和 hooks 注入能力。

![AgentRuntime 架构](docs/assets/agent-runtime-architecture.svg)

一次任务是一个独立 run session。Runtime 加载上下文后，把当前 request、可用工具 schema、历史 observation 和 context section 交给 ReAct planner。Planner 可以返回 `tool_use` 或 `final_answer`；runtime 对工具调用执行权限检查、确认、路由、审计、observation 压缩和 streaming event 输出。接近上下文预算时，runtime 会通过可注入 compactor 做压缩。
...
App 的输入层支持文本、附件和语音转写；ViewModel 把请求交给 App Services 组装 prompt、context 和 tool list。`LuminaModelRuntime` 提供 Gemma4 planner 与 BGE embedding 的 Core ML 接入，`PersonalMemory` 提供本地检索和持久化，`LuminaMarkdownUI` 负责完整 Markdown AST 渲染。工具执行结果会以用户可读摘要回到 ReAct loop，并最终渲染成 Markdown。
...
- `LuminaModelRuntime`：模型工程层，负责 Gemma4 Core ML planner、BGE embedding、tokenizer、模型资产校验和推理 adapter。
...
- 可选：`xcodegen`。当前仓库已经包含 `Lumina.xcodeproj`，只有修改 `project.yml` 后才需要重新生成。
...
- Gemma4 planner：`Resources/Models/Gemma4Planner/`
- BGE embedding：`Resources/Models/BGETextEmbedding.mlmodelc`
- BGE tokenizer：`Resources/Models/BGETextEmbedding-tokenizer.json`

默认 Gemma4 context length 是 `12000`。脚本会调用 `scripts/validate_gemma4_bundle.py` 校验模型配置、RoPE 表和上下文长度。模型资产体积较大，已经通过 `.gitignore` 排除，不应提交到仓库。

开发时可以用环境变量覆盖模型查找路径：

```bash
export LUMINA_GEMMA4_PLANNER_MODEL=/absolute/path/Gemma4Planner.mlmodelc
...
如果修改了 `project.yml`，重新生成 Xcode 工程：
...
确认 `Resources/Models/` 下存在 `Gemma4Planner/`、`BGETextEmbedding.mlmodelc` 和 `BGETextEmbedding-tokenizer.json`。如果只想验证非模型路径，测试会在未设置模型环境变量时跳过真实模型用例。
...
### 修改 `project.yml` 后 Xcode 看不到新文件怎么办？
