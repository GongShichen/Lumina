...
Lumina 是一个本地优先的 iOS 个人助手工程。它把端侧模型、ReAct agent runtime、App tools、Personal Memory 和 SwiftUI 体验拆成独立模块：模型负责理解和生成结构化 ReAct 步骤，`LuminaAgentRuntime` 只负责通用执行循环，真正的日历、提醒、文件、通讯录、位置、健康、天气等动作由 App 层注册的真实工具完成。
...
`LuminaAgentRuntime` 是纯 runtime framework。它不包含 App 业务逻辑、不内置 memory 策略、不写 system prompt，也不绑定具体模型实现。App 或其它宿主通过 request、context provider、step generator、tool registry、permission gate、confirmation coordinator、audit logger 和 hooks 注入能力。

![LuminaAgentRuntime 架构](docs/assets/agent-runtime-architecture.svg)
...
App 的输入层支持文本、附件和语音转写；ViewModel 把请求交给 App Services 组装 prompt、context 和 tool list。`LuminaModelRuntime` 提供 MiniCPM-V 4.6 ReAct model 与 BGE embedding 的本地接入，`PersonalMemory` 提供本地检索和持久化，`LuminaMarkdownUI` 负责完整 Markdown AST 渲染。工具执行结果会以用户可读摘要回到 ReAct loop，并最终渲染成 Markdown。

## 工程组成

- `LuminaAgentRuntime`：通用 ReAct runtime，包含 run/stream、tool router、permission gate、confirmation、audit、rollback、hooks、context compaction。
- `PersonalMemory`：本地 memory store，负责 ingest、chunk、metadata filter、embedding search、recent/stat/delete 等存储和检索能力。
- `LuminaModelRuntime`：模型工程层，负责 MiniCPM-V 4.6 ReAct model、BGE embedding、模型资产校验、streaming 推理 adapter 和指标采集。
...
- MiniCPM-V 4.6 ReAct model：`Resources/Models/MiniCPMV46ReActModel/`
- BGE embedding：`Resources/Models/BGETextEmbedding.mlmodelc`
- BGE tokenizer：`Resources/Models/BGETextEmbedding-tokenizer.json`

默认 MiniCPM-V 4.6 context length 是 `16000`。`scripts/setup_models.sh` 会下载官方 `openbmb/MiniCPM-V-4_6-gguf` 的 `F16` 文本模型和 `mmproj-model-f16.gguf`，并生成 `model_config.json`。模型资产体积较大，已经通过 `.gitignore` 排除，不应提交到仓库。推理入口统一走 `LuminaModelRuntimeCore` 原生 C++ engine：iOS 优先接 ANE 后端，macOS / Mac Catalyst 优先接 MPS 后端。

开发时可以用环境变量覆盖模型查找路径：

```bash
export LUMINA_MINICPMV46_MODEL=/absolute/path/MiniCPMV46ReActModel
export LUMINA_MINICPMV46_BACKEND=mps
...
- Runtime 纯粹：`LuminaAgentRuntime` 不知道 App 业务、不读取 memory、不保存用户资料。
...
确认 `Resources/Models/` 下存在 `MiniCPMV46ReActModel/`、`BGETextEmbedding.mlmodelc` 和 `BGETextEmbedding-tokenizer.json`。如果只想验证非模型路径，测试会在未设置模型环境变量时跳过真实模型用例。开发时可以用 `LUMINA_MINICPMV46_BACKEND=ane|mps|automatic` 指定 MiniCPM-V 后端偏好。
...
### 为什么不是直接把业务写进 LuminaAgentRuntime？

为了让 runtime 可复用、可测试、可移植。`LuminaAgentRuntime` 只定义通用 ReAct、tool、permission、confirmation、audit、hook 和 streaming 机制；Lumina App 负责注入 system prompt、工具、模型和平台策略。这样同一个 runtime 可以被其它 App 或 framework 宿主复用。
